...
describe('BtcAccount', function () {
  ...
  it('clearDatabase', async () => {
    let indexedDB = new IndexedDB('0101' + D.test.txWalletId)
  ...
  it('getSuggestedFee', () => {
    ...
    Number(fee[D.fee.economic]).should.above(0)
    Number(fee[D.fee.normal]).should.at.least(Number(fee[D.fee.economic]))
    Number(fee[D.fee.fast]).should.at.least(Number(fee[D.fee.normal]))
  })
